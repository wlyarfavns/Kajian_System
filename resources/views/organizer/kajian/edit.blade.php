<x-organizer-layout>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        <form action="{{ route('organizer.kajian.update', $kajian->slug) }}" method="POST" enctype="multipart/form-data" data-turbo="false" class="p-6 sm:p-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="mosque_name" id="mosque_name" class="w-full" required placeholder="Ketik nama masjid baru atau pilih dari daftar...">
                            ...
                            @foreach($mosques as $mosque)
                                <option value="{{ $mosque->name }}" {{ old('mosque_name', $kajian->mosque->name ?? '') == $mosque->name ? 'selected' : '' }}>{{ $mosque->name }} ({{ Str::limit($mosque->address, 50) }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2">
                        <div>
                            ...
                            <input type="text" name="latitude" id="latitude" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50" value="{{ old('latitude', $kajian->latitude) }}">
                            @error('latitude') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="longitude" class="block text-sm font-medium text-brand-ink mb-1">Longitude</label>
                            <input type="text" name="longitude" id="longitude" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50" value="{{ old('longitude', $kajian->longitude) }}">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="url" name="google_maps_url" id="google_maps_url" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50" placeholder="https://maps.app.goo.gl/..." value="{{ old('google_maps_url', $kajian->google_maps_url) }}">

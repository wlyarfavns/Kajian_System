<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center">
            <a href="{{ route('admin.kajian.index') }}" class="mr-4 text-gray-400 hover:text-gray-600">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        <form action="{{ route('admin.kajian.store') }}" method="POST" enctype="multipart/form-data" data-turbo="false" class="p-6 sm:p-8">
            @csrf
            ...
            <div class="mb-8 border-b border-gray-200 pb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label for="organizer_id" class="block text-sm font-medium text-brand-ink mb-1">Penyelenggara (Organizer) <span class="text-brand-danger">*</span></label>
                        <select name="organizer_id" id="organizer_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50" required>
                            <option value="">-- Pilih Penyelenggara --</option>
                            @foreach($organizers as $org)
                                <option value="{{ $org->id }}" {{ old('organizer_id') == $org->id ? 'selected' : '' }}>{{ $org->name }}</option>
                            @endforeach
                        </select>
                        @error('organizer_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
            ...
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-4 pt-4 border-t border-gray-200 mt-6">
                <a href="{{ route('admin.kajian.index') }}" class="mt-3 sm:mt-0 px-6 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-brand-ink bg-white hover:bg-gray-50 focus:outline-none text-center">
                    Batal
                </a>

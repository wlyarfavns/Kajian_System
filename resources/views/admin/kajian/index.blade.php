                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-6 py-4 text-xs font-semibold text-brand-ink-soft uppercase tracking-wider">Judul & Info</th>
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-brand-ink">{{ $kajian->title }}</span>
                                        <span class="text-xs text-gray-500 mt-1 flex items-center">
                                            <i data-lucide="calendar" class="w-3 h-3 mr-1"></i> 
                                            {{ \Carbon\Carbon::parse($kajian->start_at)->format('d M Y, H:i') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        @if($kajian->organizer->logo)
                                            <img src="{{ asset('storage/' . $kajian->organizer->logo) }}" class="h-6 w-6 rounded-full object-cover border border-gray-200 mr-2">
                                        @else
                                            <div class="h-6 w-6 rounded-full bg-brand-emerald-100 flex items-center justify-center text-brand-emerald-800 text-xs font-bold mr-2">
                                                {{ substr($kajian->organizer->name, 0, 1) }}
                                            </div>
                                        @endif
                                        <span class="text-sm font-medium text-brand-ink">{{ $kajian->organizer->name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($kajian->status === 'pending')
                                    ...
                                    @elseif($kajian->status === 'published')
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200">
                                            <i data-lucide="check-circle-2" class="w-3.5 h-3.5 mr-1"></i> Published
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200">
                                            <i data-lucide="x-circle" class="w-3.5 h-3.5 mr-1"></i> Ditolak
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    @php
                                        $kajianData = [
                                            'id' => $kajian->id,
                                            'title' => $kajian->title,
                                            'poster' => $kajian->poster ? asset('storage/' . $kajian->poster) : null,
                                    ...
                                    @endphp
                                    <button type="button" @click="openModal({{ json_encode($kajianData) }})" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-sm font-medium rounded-md text-brand-ink bg-white hover:bg-gray-50 transition" title="Lihat Detail & Moderasi">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-brand-ink-soft">
                                    Belum ada kajian baru yang dikirimkan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

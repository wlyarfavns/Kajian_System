    ...
    <div x-data="{ deleteModalOpen: false, deleteFormAction: '' }" class="bg-white border border-gray-200 rounded-xl shadow-sm">
        <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            <div class="mt-4 sm:mt-0 flex flex-col sm:flex-row gap-3 items-center w-full sm:w-auto">
                <form action="{{ route('admin.mosque.index') }}" method="GET" class="relative w-full sm:w-64" data-turbo="false">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau alamat..." spellcheck="false" autocomplete="off" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-emerald-900 focus:border-brand-emerald-900 transition">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i data-lucide="search" class="w-4 h-4 text-gray-400"></i>
                    </div>
                </form>
                <a href="{{ route('admin.mosque.create') }}" data-turbo="false" class="inline-flex items-center justify-center px-4 py-2 w-full sm:w-auto bg-brand-emerald-900 text-white text-sm font-medium rounded-lg hover:bg-brand-emerald-950 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-emerald-900 shrink-0">
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-center">
            {{ $mosques->links('vendor.pagination.custom-dark') }}
        </div>
